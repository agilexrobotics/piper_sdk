...
| Variable | Type |Description                                                          |
| ----- | ---- |------------------------------------------------------------ |
|`arm_can`|`C_STD_CAN`|The instantiation of classes in `can_encapsulation.py` serves as the hardware data reception layer, registering data processing callback functions.|
|`parser`|`C_PiperParserBase`|Instantiation of the data protocol parsing class, currently using the V1 version protocol parsing. It serves as the base class for data parsing.|
|`__arm_time_stamp`|`ArmTimeStamp`|The instantiation of the timestamp class is a private class of `C_PiperInterface`. It handles timestamps for robotic arm variables that are composed of multiple CAN frames, such as joint angles.|
|`__arm_status`|`ArmMsgStatus`|Instance of the robotic arm status message, containing the status information of the robotic arm.|
...
| ----- | ---- |------------------------------------------------------------ |-------------------|--------------------------- |--------|
|`ConnectPort`|Getter Method|This function is used to open the CAN message reading function. It calls the function to read CAN messages, creates a reading thread, and starts the thread.|Used to open the data processing thread after externally instantiating the class.|None|None|
...
|`GetAllMotorMaxAccLimit`|Getter Method|Get the angle limits and maximum speeds of all joint motors of the robotic arm, using the mutex `__arm_all_motor_angle_limit_max_spd_mtx` to ensure thread safety.||||
...
|`EndPoseCtrl`|Ctrl MethodCtrl Method|0x152, 0x153, 0x154, control the end position and posture of the robotic arm. This function consolidates the three functions below; note that the x, y, z values sent by this function are in units of 0.001 mm, and the rx, ry, rz values are in units of 0.001 degrees. For example, if the robotic arm needs to move 0.3 m in the x-axis direction, the data sent should be 0.3 * 100 * 1000 = 30000.|Called from outside the class|`X (float)`, `Y (float)`, `Z (float)`, `RX (float)`, `RY (float)`, `RZ (float)``X (float)`, `Y (float)`, `Z (float)`, `RX (float)`, `RY (float)`, `RZ (float)`|NoneNone|
|`__CartesianCtrl_XY`|Ctrl MethodCtrl Method|Control the XY axis motion of the robotic arm in Cartesian coordinates.|Called within the class (private)|`X`X (float)`, `Y (float)`|None|
|`__CartesianCtrl_ZRX`|Ctrl Method|Control the Z-axis and RX-axis movement of the robotic arm in Cartesian coordinates.|Called within the class (private)|`Z (float)`, `RX (float)`|None|
|`__CartesianCtrl_RYRZ`|Ctrl Method|Control the RY-axis and RZ-axis movement of the robotic arm in Cartesian coordinates.|Called within the class (private)|`RY (float)`, `RZ (float)`|None|
|`JointCtrl`|Ctrl Method|0x155, 0x156, 0x157, control the movement of the six joints of the robotic arm. Note that the values sent by this function are in units of 0.001 degrees. For example, to move joint 1 by 90 degrees (1.57 radians), the data sent should be 90 * 1000. If using radians, it should be 1.57 * 1000 * (180/3.14).|Called from outside the class|`joint_1 (float)`, `joint_2 (float)`, `joint_3 (float)`, `joint_4 (float)`, `joint_5 (float)`, `joint_6 (float)`|None|
|`__JointCtrl_12`|Ctrl Method|Control the movement of joints 1 and 2 of the robotic arm.|Called within the class (private)|`joint_1 (float)`, `joint_2 (float)`|None|
|`__JointCtrl_34`|Ctrl Method|Control the movement of joints 3 and 4 of the robotic arm.|Called within the class (private)|`joint_3 (float)`, `joint_4 (float)`|None|
|`__JointCtrl_56`|Ctrl Method|Control the movement of joints 5 and 6 of the robotic arm.|Called within the class (private)|`joint_5 (float)`, `joint_6 (float)`|None|
...
|`MotorAngleLimitMaxSpdSet`|Ctrl Method|Set the angle limit and maximum speed for the motor.|Called from outside the class|`motor_num (int)`, `max_angle_limit (float)`, `min_angle_limit (float)`, `max_jonit_spd (float)`|None|
|`JointConfig`|Ctrl Method|Configure joint parameters.|Called from outside the class|`joint_num (int)`, `set_zero (int)`, `acc_param_is_effective (int)`, `set_acc (int)`, `clear_err (int)`|None|
...
|`EndSpdAndAccParamSet`|Ctrl Method|Set the end speed and acceleration parameters of the robotic arm.|Called from outside the class|`end_max_linear_vel (float)`, `end_max_angular_vel (float)`, `end_max_linear_acc (float)`, `end_max_angular_acc (float)`|None|
|`CrashProtectionConfig`|Ctrl Method|Configure the collision protection level of the robotic arm.|Called from outside the class|`jonit_1_protection_level (int)`, `jonit_2_protection_level (int)`, `jonit_3_protection_level (int)`, `jonit_4_protection_level (int)`, `jonit_5_protection_level (int)`, `jonit_6_protection_level (int)`|None|
