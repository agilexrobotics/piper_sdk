...
|ArmJointAndGripper|机械臂关节角度和夹爪二次封装类，将夹爪和关节角度信息放在一起，增加时间戳。|
...
|ArmJointAndGripperCtrl|机械臂关节角度和夹爪控制消息类，增加时间戳。|
...
|`__arm_joint_gripper_msgs`|`ArmJointAndGripper`|机械臂关节角度反馈消息的实例，包含各关节的状态信息。|
...
|`__arm_joint_gripper_ctrl_msgs`|`ArmJointAndGripperCtrl`|变量为读取主动臂给从动臂的关节角和夹爪消息|
...
|GetArmJointGripperMsgs|Getter Method|获取机械臂关节和夹爪的状态消息。使用互斥锁__arm_joint_gripper_msgs_mtx保证线程安全。|obj.GetArmJointGripperMsgs()|None|self.__arm_joint_gripper_msgs|
...
|GetCurrentMotorAngleLimitMaxVel|Getter Method||obj.GetCurrentMotorAngleLimitMaxVel()|None|self.__feedback_current_motor_angle_limit_max_vel|
...
|GetArmJointGripperCtrlMsgs|Getter Method|获取机械臂关节和夹爪控制消息。使用互斥锁__arm_joint_gripper_ctrl_msgs_mtx保证线程安全。|obj.GetArmJointGripperCtrlMsgs()|None|self.__arm_joint_gripper_ctrl_msgs|
...
|UpdateArmJointGripperState|Getter Method|更新关节和夹爪状态，关节角单位为0.001度，夹爪反馈的行程单位为0.001mm|类内使用|msg:PiperMessage|self.__arm_joint_gripper_msgs|
...
|UpdateArmJointGripperStateCtrl|Getter Method|更新关节和夹爪状态（为主臂发送的消息）|类内使用|msg:PiperMessage|self.__arm_joint_gripper_ctrl_msgs|
...
|EndPoseCtrl|Ctrl Method|0x152,0x153,0x154,控制机械臂的末端位置和姿态。是下面三个函数的汇总,需要注意的是，该函数发送出去的x、y、z数值单位为0.001mm,rx、ry、rz数值单位为0.001度假如需要机械臂向x轴方向运行0.3m，则发送的数据应为0.3*1000=300|类外调用|`X (float)`, `Y (float)`, `Z (float)`, `RX (float)`, `RY (float)`, `RZ (float)`|None|
