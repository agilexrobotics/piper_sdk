...
def main():
    ...
    # Switch to MIT mode (common usage) :contentReference[oaicite:3]{index=3}
    ...
    while True:
        ...
        # Breakaway detection (joint1)
        if abs(q[0] - q0[0]) >= BREAKAWAY_THRESH_RAD:
            print(f"[INFO] breakaway: t={t_s:.3f}s, tau_cmd={tau_cmd:.3f} Nm, dq={q[0]-q0[0]:.4f} rad")
            break
        ...
        # MIT control:
        # Joint1: torque sweep
        piper.JointMitCtrl(1, 0.0, 0.0, KP_J1, KD_J1, tau_cmd)
        # Joints2..6: hold at 0 rad
        for jid in range(2, 7):
    ...
    if len(log_data) > 1:
        ...
        filename = f"log_joint1_static_experiment_minus{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
