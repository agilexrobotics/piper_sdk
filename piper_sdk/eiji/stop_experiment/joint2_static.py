...
def main():
    ...
    # 3. 初期角度に移動
    print("初期角度へ移動します...")
    piper.JointCtrl(0, 65*1000, 0, 0, 0, 0)  # joint2 = 65 deg
    # アームが目標位置に到達するまで待機
    print("移動完了を待っています...")
    ...
    while True:
        ...
        # Breakaway detection (joint2)
        if abs(q[1] - q0[1]) >= BREAKAWAY_THRESH_RAD:
            print(f"[INFO] breakaway: t={t_s:.3f}s, tau_cmd={tau_cmd:.3f} Nm, dq={q[1]-q0[1]:.4f} rad")
            break
        ...
        # MIT control:
        # Joint2: torque sweep
        piper.JointMitCtrl(2, 0.0, 0.0, KP_J2, KD_J2, tau_cmd)
        # Joints1,3..6: hold at 0 rad
    ...
    if len(log_data) > 1:
        ...
        filename = f"log_joint2_static_experiment_minus_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
