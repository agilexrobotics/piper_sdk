...
def main():
    ...
    # 3. 初期角度に移動
    print("初期角度へ移動します...")
    piper.JointCtrl(0, 0, -80*1000, 0, 0, 0)  # joint3 = -80 deg
    # アームが目標位置に到達するまで待機
    print("移動完了を待っています...")
    ...
    while True:
        ...
        # Breakaway detection (joint3)
        if abs(q[2] - q0[2]) >= BREAKAWAY_THRESH_RAD:
            print(f"[INFO] breakaway: t={t_s:.3f}s, tau_cmd={tau_cmd:.3f} Nm, dq={q[2]-q0[2]:.4f} rad")
            break
        ...
        # MIT control:
        # Joint3: torque sweep
        piper.JointMitCtrl(3, 0.0, 0.0, KP_J3, KD_J3, tau_cmd)
        # Joints1,2,4,5,6: hold at 0 rad
        for jid in (1, 2, 4, 5, 6):
    ...
    if len(log_data) > 1:
        ...
        filename = f"log_joint3_static_experiment_minus_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
