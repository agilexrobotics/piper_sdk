...
class Piper():
    ...
    def get_gripper_states(self) -> Tuple[Tuple[float, float], float, float]:
        if self._interface.get_connect_status():
            ...
            result = (
                round(tmp.gripper_state.grippers_angle / 1000, 6),
    ...
    def move_gripper(self, gripper_val:float, effort:float = None):
        ...
        if effort is None:
            self._interface.GripperCtrl(round(self.__gripper_val * 1000), round(self.__effort * 1000), self.__enable_code, 0)
        else:
            self._interface.GripperCtrl(round(self.__gripper_val * 1000), round(effort * 1000), self.__enable_code, 0)
