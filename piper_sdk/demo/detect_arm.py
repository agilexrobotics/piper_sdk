...
def display_table(can_port, refresh_interval):
    ...
    # 如果距离上一次执行已超过限制间隔，就执行操作
        ...
        print(f"+{'-'*101}+\n"
              f"|{'JointState':<10}|{'J1':^15}{'J2':^15}{'J3':^15}{'J4':^15}{'J5':^15}{'J6':^15}|\n"
              f"+{'-'*10:^10}+{'-'*90:^}+\n"
              f"|{'pos(deg)':<10}|"
        ...
        print(
              ...
              f"{'m/s^2':<5}|"
              f"{'max_angular_acc':<17}"
              f"{round(piper.GetCurrentEndVelAndAccParam().current_end_vel_acc_param.end_max_angular_acc*1e-3, 3):^7}"
              f"{'rad/s^2':<8}|"
              )
        print(f"+{'-'*101}+\n"
              f"{'Gripper&Teaching':<102}|"
              
              f"\n"
              f"{'gripper_pos':<20}{round(piper.GetArmGripperMsgs().gripper_state.grippers_angle*1e-3, 3):<6}|"
              f"{'Status code :':<59}"
              f"{'|':>17}"
              f"\n"
              f"{'gripper_effort':<20}{round(piper.GetArmGripperMsgs().gripper_state.grippers_effort*1e-3, 3):<6}|"
              ...
              f"{'|':>17}"
              f"\n"
              f"{'teaching_per':<20}{piper.GetGripperTeachingPendantParamFeedback().arm_gripper_teaching_param_feedback.teaching_range_per:<6}|"
              ...
              f"{'|':>17}"
              f"\n"
              f"{'max_range_config':<20}{piper.GetGripperTeachingPendantParamFeedback().arm_gripper_teaching_param_feedback.max_range_config:<6}|"
              ...
              f"{'driver_error_status':<23}{str(piper.GetArmGripperMsgs().gripper_state.foc_status.driver_error_status):<6}"
                f"{'|':>17}"              
              f"\n"
              f"{'teaching_friction':<20}{piper.GetGripperTeachingPendantParamFeedback().arm_gripper_teaching_param_feedback.teaching_friction:<6}|"
