...
class C_PiperParserV2(C_PiperParserBase):
    ...
    def parse_can_message(self, can_id, can_data, msg):
        ...
        handler = ProtocolMapping.decodemsg_get_handler(can_id, "V2")  # 通过 ProtocolMapping 获取
        if handler:
            handler(self, can_id, can_data, msg, "V2")  # 执行解析函数
            return True
        else:
    ...
    def parse_msg_type(self, msg_type_, msg, tx_can_frame):
        handler = ProtocolMapping.encodemsg_get_handler(msg_type_, "V2")  # 通过 ProtocolMapping 获取
        # print(msg_type_)
        if handler:
            handler(self, msg_type_, msg, tx_can_frame, "V2")  # 执行解析函数
            return True
        else:
